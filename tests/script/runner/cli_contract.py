...
with tempfile.TemporaryDirectory(prefix="grapple cli '") as directory:
    ...
    for script in (lua, ruby):
        for name, value in [("window-mode", "fullscreen-exclusive"), ("vsync", "off"),
            ...
            run("--" + name, value, str(script), contains="GAME_RAN")
            run("--" + name + "=" + value, str(script), contains="GAME_RAN")
        run(str(script), "--", "--vsnyc", "on", contains="--vsnyc\non")
        run(str(script), "--level", "3", contains="--level\n3")
        run("--crt", "0", "--crt", "1", str(script), contains="GAME_RAN")
        for invalid in [("--vsnyc=off",), ("--fullscreen",), ("--windowed",),
            ...
            output = run(*invalid, str(script), status=2)
            assert "GAME_RAN" not in output
        run("--reduced-flashing", str(script), status=2, contains="on")
    run("--max-fps", status=2, contains="requires a value")
    run("-l", "ruby", str(lua), status=2, contains="language must agree")
    run("--help", contains="--window-mode")
    ...
    run("-l", "lua", status=2)
    run("--list-backends", str(lua), status=2)
