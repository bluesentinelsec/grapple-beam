...
extern "C"
{
    ...
    typedef struct Grapple_GraphicsSettings
    {
        ...
        float ui_scale;
        /** Exclusive fullscreen mode; zero dimensions/rate choose desktop defaults. */
        int fullscreen_width, fullscreen_height;
        float refresh_rate;
        bool primary_display;  /**< Select the current primary display at application time. */
        bool effects_disabled; /**< Gate stylistic effects while retaining image/accessibility
                                  correction. */
    ...
    /** @brief Font point size honoring the engine UI-scale preference.
     * @param engine Engine, or NULL for scale one. @param points Base points; nonpositive uses 15.
     * @return Scaled size for Grapple_OpenUi/CreateGui. Recreate the UI after changing UI scale.
     * Script Grapple.ui(engine) calls this automatically; custom fixed layouts should scale their
     * dimensions as well. */
    extern float Grapple_EngineUiPoints(Grapple_Engine *engine, float points);
     ...
     */
    extern void Grapple_GraphicsResolve(Grapple_GraphicsSettings *out, int argc, char *const *argv,
    ...
    extern bool Grapple_GraphicsLoadTomlFile(Grapple_GraphicsSettings *settings, const char *path);
