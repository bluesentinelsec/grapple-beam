...
#include <lua.h>
     ...
     * @param path Settings source file; trusted executable code.
     * @return Success; Lua must return a table, Ruby must evaluate to a hash.
     * No engine is created. All values and origins are copied into settings. */
    extern bool Grapple_LoadSettingsScript(Grapple_Settings *settings, const char *language,
                                           const char *path);

    /** @brief Install borrowed argv for embedded legacy engine constructors.
     * @param argc Argument count. @param argv Borrowed array; retain until cleared.
     * The runner uses Grapple_SetLaunchSettings for typed configuration. */
